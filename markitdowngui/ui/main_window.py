...
class MainWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        self.themeToggleButton.setToolTip(
            self.translate("menu_dark_mode") or "Dark Mode"
        )
        ...
        self.settingsBar.plugins_toggled.connect(
            lambda _: self._invalidate_preview_cache()
        )
        self.settingsBar.endpoint_changed.connect(
            lambda _t: self._invalidate_preview_cache()
        )

        # Output area component
        self.outputPanel = OutputPanel(
            self.translate, self.settings_manager.get_save_mode()
        )
        # wire save mode to settings
        self.outputPanel.combined_toggle.toggled.connect(
            self.settings_manager.set_save_mode
        )
    ...
    def update_menu_bar_texts(self):
        ...
        # Dark mode toggle
        self.darkModeAction = viewMenu.addAction(
            self.translate("menu_dark_mode") or "Dark Mode"
        )
        ...
        # Language menu
        self.languageMenu = viewMenu.addMenu(
            self.translate("menu_language") or "Language"
        )
        ...
        # Settings menu
        settingsMenu = self.menuBar.addMenu(
            self.translate("menu_settings") or "Settings"
        )
        formatAction = settingsMenu.addAction(
            self.translate("menu_format_settings") or "Format Settings"
        )
        formatAction.triggered.connect(self.show_format_settings)

        # Help menu
        helpMenu = self.menuBar.addMenu(self.translate("menu_help") or "Help")
        shortcutsAction = helpMenu.addAction(
            self.translate("menu_keyboard_shortcuts") or "Keyboard Shortcuts"
        )
        shortcutsAction.triggered.connect(self.show_shortcuts)

        checkUpdateAction = helpMenu.addAction(
            self.translate("menu_check_updates") or "Check for Updates"
        )
    ...
    def setup_shortcuts(self):
        ...
        QShortcut(
            QKeySequence("Ctrl+P"),
            self,
            lambda: self.convertControls.pause_button.toggle(),
        )
        ...
        QShortcut(
            QKeySequence("Ctrl+Shift+L"), self, self.clear_file_list
        )  # New shortcut for clear all
    ...
    def apply_theme(self):
        ...
        palette = (
            apply_dark_theme(QPalette()) if self.isDarkMode else apply_light_theme()
        )
        ...
        try:
            ...
            try:
                ...
                QMessageBox.critical(
                    ...
                    self.translate("markitdown_config_error_message").format(
                        error=str(e)
                    ),
                )
                return
            ...
            try:
                self.worker = ConversionWorker(
                    [md, valid_files, settings], self.convertControls.batch_spin.value()
                )
                ...
                self.convertControls.progress.setFormat(
                    self.translate("conversion_starting_message")
                )
                ...
                QMessageBox.critical(
                    ...
                    self.translate("conversion_start_error_message").format(
                        error=str(e)
                    ),
                )
    ...
    def save_combined_output(self):
        ...
        if output_path:
            try:
                self.file_manager.save_markdown_file(
                    output_path, self.outputPanel.get_text()
                )
                AppLogger.info(
                    self.translate("auto_save_backup_log").format(path=output_path)
                )
                ...
                QMessageBox.critical(
                    ...
                    self.translate("error_saving_combined_message").format(
                        error=str(e)
                    ),
                )
    ...
    def save_individual_outputs(self):
        ...
        output_dir = QFileDialog.getExistingDirectory(
            self, self.translate("select_directory_title")
        )
        ...
        if output_dir:
                ...
                try:
                    ...
                    while os.path.exists(output_path):
                        output_path = os.path.join(
                            output_dir, f"{base_name}_{counter}.md"
                        )
                        counter += 1
                    ...
                    success_count += 1
                    AppLogger.info(
                        self.translate("added_file_to_recent_log").format(
                            file=output_path
                        )
                    )
                except Exception as e:
                    AppLogger.error(
                        self.translate("error_handling_new_file_log").format(
                            error=str(e)
                        )
                    )
            ...
            if success_count > 0:
                QMessageBox.information(
                    ...
                    self.translate("save_individual_complete_message").format(
                        count=success_count, dir=output_dir
                    ),
                )
                ...
                self.settings_manager.set_recent_outputs(
                    self.file_manager.update_recent_list(
                        output_dir, self.settings_manager.get_recent_outputs()
                    )
                )
    ...
    def perform_auto_save(self):
        ...
        if hasattr(self, "conversionResults") and self.conversionResults:
            ...
            try:
                self.file_manager.save_markdown_file(
                    backup_path, self.outputPanel.get_text()
                )
                AppLogger.info(
                    self.translate("auto_save_backup_log").format(path=backup_path)
                )
            except Exception as e:
                AppLogger.error(
                    self.translate("auto_save_failed_log").format(error=str(e))
                )
    ...
    def _update_theme_toggle_icon(self):
        ...
        try:
            ...
            icon = make_tinted_svg_icon(
                resources_dir, icon_name, tint_color, desired_size
            )
    ...
    def update_preview(self, current, previous):
        ...
        try:
            ...
            AppLogger.error(
                self.translate("preview_error_log").format(error=str(e)), filepath
            )
    ...
    def toggle_pause(self, paused):
        ...
        if hasattr(self, "worker") and self.worker:
            ...
            self.convertControls.pause_button.setText(
                self.translate("resume_button")
                if paused
                else self.translate("pause_button")
            )
            AppLogger.info(
                self.translate("conversion_paused_log")
                if paused
                else self.translate("conversion_resumed_log")
            )
